...
class MongoCollectionAdapter:
    ...
    def _format_output(self, documents: List[Dict]) -> Dict:
        """
        Formats a list of MongoDB documents into the exact dictionary structure that
        ChromaDB's .query() method returned. This is critical for ensuring the AI's
        Synthesizer prompt receives data in the format it was trained on.

        Args:
            documents: A list of document dictionaries from a MongoDB find() query.

        Returns:
            A dictionary with 'documents', 'metadatas', and 'ids' keys, formatted for AI compatibility.
        """
        if not documents:
        ...
        for doc in documents:
            # The AI's Synthesizer expects a "content" field for its main analysis.
            # We use the 'content' field generated by the `prepare_mongo_data.py` script.
            docs_list.append(doc.get("content", "No content available."))
            
            # The entire MongoDB document is treated as the "metadata" payload.
            # We convert all values to strings to prevent serialization errors later on.

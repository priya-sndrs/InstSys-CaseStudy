...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validatePassword(form.password)) {
      setError("❌ Password must be 8-16 characters long and include uppercase, lowercase, numbers, and special characters.");
      return;
    }
        ...
        <div className='flex flex-col justify-center gap-10 items-center bg-white w-[35vw] h-fit pt-[2%] pb-[1%] transition-all duration-300 rounded-xl'>
          ...
          {showSuccess && (
            <div className="bg-green-200 text-green-800 p-3 rounded-lg">
              ...
              <button onClick={handlePopupClose} className="ml-2 underline">Go to Login</button>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className='flex flex-col gap-3 justify-center items-center'>
            ...
            <div className='w-full flex flex-col gap-4 items-center justify-center'>
              ...
              <button type="button" onClick={() => window.location.href = "/login"} className='font-sans font-medium underline text-[clamp(0.6rem,1.3vw,1.2rem)] cursor-pointer'>← Already Have an Account?</button>
            </div>
          </form>
        </div>
      </div>

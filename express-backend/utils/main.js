...
class CORScheduleManager {
  ...
  async storeCORSchedule(corData) {
    try {
      ...
      const scheduleDoc = {
        // Identification
        schedule_id: `COR_${corData.metadata.department}_${corData.metadata.course}_Y${corData.metadata.year_level}_${corData.metadata.section}_${Date.now()}`,
        
        // Program Information
        course: corData.metadata.course,
        section: corData.metadata.section,
        year_level: corData.metadata.year_level,
  ...
  /**
   * Get COR schedules by filters
   */
  async getCORSchedules(filters = {}) {
    try {
      const dept = filters.department ? filters.department.toLowerCase() : 'ccs';
      const collection = this.db.db.collection(`schedules_${dept}`);
      
      // Build query
      const query = { data_type: 'cor_schedule' };
      if (filters.course) query.course = filters.course;
      if (filters.year_level) query.year_level = filters.year_level;
  ...
  async storeStudentGrades(gradesData) {
    try {
      ...
      const gradesDoc = {
        ...
        year_level: existingStudent.year,
